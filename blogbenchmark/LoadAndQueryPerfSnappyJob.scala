...
object LoadAndQueryPerfSnappyJob extends SnappySQLJob {
  override def runSnappyJob(snSession: SnappySession, jobConfig: Config): Any = {

    val snc = snSession.sqlContext
    val pw = new PrintWriter(new FileOutputStream(new File("LoadAndQueryPerfSnappyJob" +
        ".out"),
      true))
    Try {

      val airlineDataFrame = CommonObject.getAirlineDataFrame(snc)
      val airlineRefDataFrame = CommonObject.getAirlineRefDataFrame(snc)

      snc.dropTable("airline", ifExists = true)
      snc.dropTable("airlineref", ifExists = true)

      val p1 = Map("PERSISTENCE" -> "NONE")
      val snappyContext = snc.asInstanceOf[SnappyContext]
      snappyContext.createTable("airline", "column", airlineDataFrame.schema, p1)
      ...
      CommonObject.runQueries("SnappyEmbedded", pw, snc, queries, 1)
      start = System.currentTimeMillis
